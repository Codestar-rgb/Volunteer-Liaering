...
/**
 * EntityAssimilatedCow - The assimilated cow entity (Feral stage).
 * <p>
 * Fully transformed cow host with enhanced strength and aggression.
 */
public class EntityAssimilatedCow extends EntityParasiteBase {

    private static final EntityDataAccessor<Integer> ASSIMILATION_PROGRESS =
            SynchedEntityData.defineId(EntityAssimilatedCow.class, EntityDataSerializers.INT);
    ...
    public EntityAssimilatedCow(EntityType<? extends EntityAssimilatedCow> type, Level world) {
        super(type, world);
    ...
    public boolean doHurtTarget(net.minecraft.world.entity.Entity target) {
        ...
        if (result && target instanceof net.minecraft.world.entity.LivingEntity living) {
            ...
            this.playSound(ModSounds.ASSIMILATED_COW_ATTACK.get(), 1.0f, 1.0f);
        }
    ...
    private void attemptInfect(net.minecraft.world.entity.LivingEntity target) {
        ...
        if (infectionComponent != null) {
            infectionComponent.applyInfection(target, 0.4f);
        }
    }

    @Override
    protected SoundEvent getAmbientSound() {
        return ModSounds.ASSIMILATED_COW_IDLE.get();
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource source) {
        return ModSounds.ASSIMILATED_COW_HURT.get();
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.ASSIMILATED_COW_DEATH.get();
    }

    @Override
    protected void playStepSound(BlockPos pos, net.minecraft.world.level.block.state.BlockState block) {
        this.playSound(ModSounds.PARASITE_STEP.get(), 0.18f, 1.0f);
    }
