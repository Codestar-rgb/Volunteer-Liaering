...
public class ItemParasiteFlesh extends Item {
    ...
    public ItemStack finishUsingItem(ItemStack stack, Level level, LivingEntity livingEntity) {
        ...
        if (!level.isClientSide() && livingEntity instanceof Player player) {
            player.getCapability(ParasiteCapability.CAPABILITY).ifPresent(capability -> {
                // Increase infection value when consuming parasite flesh
                capability.addInfection(15);
                
                // If infection was at zero before adding (meaning they just got infected),
                // apply Corrosion effect to simulate initial infection
                if (capability.getInfectionLevel() <= 15) {
