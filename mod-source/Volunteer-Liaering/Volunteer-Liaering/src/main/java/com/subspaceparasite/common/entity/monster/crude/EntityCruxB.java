...
/**
 * 十字型 B - Crude Stage Parasite
 * 特性：快速连击流血效果
 * 
 * 属性设计说明：
 * - MAX_HEALTH: 42.0 - 中等血量
 * - MOVEMENT_SPEED: 0.28 - 较高移动速度
 * - ATTACK_DAMAGE: 7.5 - 中等攻击力，依靠攻速和流血效果
 */
public class EntityCruxB extends EntityCrudeBase {

    public EntityCruxB(EntityType<? extends EntityCrudeBase> entityType, Level worldIn) {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.SUBSRP_ENTITY_CRUXB_AMBIENT.get();
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return ModSounds.SUBSRP_ENTITY_CRUXB_HURT.get();
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.SUBSRP_ENTITY_CRUXB_DEATH.get();
    }
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return EntityCrudeBase.createAttributes()
            .add(Attributes.MAX_HEALTH, 42.0D)      // 中等血量
            .add(Attributes.MOVEMENT_SPEED, 0.28D)  // 较高移动速度
            .add(Attributes.ATTACK_DAMAGE, 7.5D);   // 中等攻击力
    }
}
