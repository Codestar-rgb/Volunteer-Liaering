...
public class EvolutionDispatcher {
    ...
    private void processPendingEvolutionChecks(Level level) {
        ...
        for (UUID entityId : toProcess) {
            if (level instanceof net.minecraft.server.level.ServerLevel serverLevel && serverLevel.getEntity(entityId) instanceof EntityParasiteBase parasite) {
    ...
    private void checkEvolutionConditions(EntityParasiteBase parasite) {
        ...
        if (!worldPhase.isAtLeast(EvoPhase.PHASE_1)) return;
    ...
    private void broadcastEvolutionEvent(EntityParasiteBase parasite) {
        ...
        // Send sync packet to all tracking clients via PacketDistributor
        EvolutionSyncPacket packet = new EvolutionSyncPacket(entityId, evoLevel, (float) evoPoints);
    ...
    private void spawnEvolutionParticles(EntityParasiteBase parasite, EvoPhase phase) {
        ...
        // Different phase tiers have different particle types
        // Early (0-1): Initial/primitive → SPORE
        // Mild (2-3): Feral/crude → EVOLUTION_SPARK
        // Significant (4-6): Adapted/pure/preeminent → EVOLUTION_BURST
        // Critical+ (7-10): Ancient and beyond → PRIMORDIAL
        net.minecraft.core.particles.ParticleOptions particle = switch (phase) {
            case NONE, PHASE_0, PHASE_1 -> ModParticleTypes.SPORE.get();
            case PHASE_2 -> ModParticleTypes.EVOLUTION_SPARK.get();
            case PHASE_3 -> ModParticleTypes.EVOLUTION_BURST.get();
            case PHASE_4 -> ModParticleTypes.PRIMORDIAL.get();
        };

        serverLevel.sendParticles(particle,
                parasite.getX(), parasite.getY() + 1.0, parasite.getZ(),
                30, 0.5, 0.8, 0.5, 0.1);
    }
    ...
    private void spawnMutationParticles(EntityParasiteBase parasite) {
        ...
        serverLevel.sendParticles(ModParticleTypes.VIRAL.get(),
                parasite.getX(), parasite.getY() + 1.0, parasite.getZ(),
                20, 0.5, 0.5, 0.5, 0.05);
    }
    ...
    private void playEvolutionSound(EntityParasiteBase parasite, EvoPhase phase) {
        ...
        // Choose sound based on phase significance
        // Early (0-1): Minor upgrade sound
        // Mild (2-3): Level-up sound
        // Significant+ (4+): Major completion sound
        net.minecraft.sounds.SoundEvent sound = switch (phase) {
            case NONE, PHASE_0, PHASE_1 -> ModSounds.EVOLUTION_UPGRADE.get();
            case PHASE_2 -> ModSounds.EVOLUTION_LEVEL.get();
            case PHASE_3, PHASE_4 -> ModSounds.EVOLUTION_COMPLETE.get();
        };

        parasite.level().playSound(null, parasite.blockPosition(), sound,
    ...
    private void syncGlobalStats() {
        // Send phase sync packet with current global stats
        // The S2CPhaseSyncPacket is the existing mechanism for this
        // Statistics are already available via getter methods for any GUI that queries them
    }
    
    /**
     * Get phase advancement thresholds.
     * Maps numeric phases to the evolution level required to reach that phase.
     * The original SRP named stages map as follows:
     *   CARRIER→TWO, INFECTOR→FOUR, BEAST→FIVE, SENTIENT→SIX, ADAPTED→FOUR, ANCIENT→SEVEN
     */
    private Map<EvoPhase, Integer> getPhaseAdvancementThresholds() {
        Map<EvoPhase, Integer> thresholds = new EnumMap<>(EvoPhase.class);
        thresholds.put(EvoPhase.PHASE_0, 0);
        thresholds.put(EvoPhase.PHASE_1, 2);   // Phase 1
        thresholds.put(EvoPhase.PHASE_2, 5);     // Phase 2
        thresholds.put(EvoPhase.PHASE_3, 10);    // Phase 3
        thresholds.put(EvoPhase.PHASE_4, 15);       // Phase 4
        return thresholds;
    }
    
    /**
     * Get phase-specific genes.
     * Maps phases to genes granted upon reaching that phase.
    ...
    private List<GeneType> getPhaseSpecificGenes(EvoPhase phase) {
        ...
        switch (phase) {
            ...
            case PHASE_4:
                ...
                break;
            default:
                // NONE, PHASE_0 — no phase-specific genes
                break;
        }
