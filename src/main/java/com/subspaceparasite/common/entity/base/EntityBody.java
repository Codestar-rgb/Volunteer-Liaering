 ...
 */
public class EntityBody extends PartEntity<EntityParasiteBase> {
    ...
    public void addAdditionalSaveData(CompoundTag compound) {
        // No additional NBT data needed for body parts
    }
    ...
    /**
     * Get the part ID.
     * Renamed from getId() to avoid overriding Entity.getId() which returns
     * the network entity ID, not the body part index.
     */
    ...
    public AABB getBodyBoundingBox() {
    ...
    public void setBodySize(float width, float height) {
        this.setBoundingBox(this.getBoundingBox().inflate(
                (width - this.getDimensions(Pose.STANDING).width) / 2.0,
                (height - this.getDimensions(Pose.STANDING).height) / 2.0,
                (width - this.getDimensions(Pose.STANDING).width) / 2.0));
