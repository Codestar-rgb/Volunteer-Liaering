...
/**
 * 静音者 - Crude Stage Parasite
 * 特性：反音波攻击沉默领域
 * 
 * 属性设计说明：
 * - MAX_HEALTH: 32.0 - 低血量，特殊功能型单位
 * - MOVEMENT_SPEED: 0.29 - 高移动速度
 * - ATTACK_DAMAGE: 5.5 - 低攻击力，主要依靠特殊能力
 */
public class EntityQuac extends EntityParasiteBase {

    public EntityQuac(EntityType<? extends EntityParasiteBase> entityType, Level worldIn) {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.SUBSRP_ENTITY_QUAC_AMBIENT.get();
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return ModSounds.SUBSRP_ENTITY_QUAC_HURT.get();
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.SUBSRP_ENTITY_QUAC_DEATH.get();
    }
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return EntityParasiteBase.createMonsterAttributes()
            .add(Attributes.MAX_HEALTH, 32.0D)      // 低血量
            .add(Attributes.MOVEMENT_SPEED, 0.29D)  // 高移动速度
            .add(Attributes.ATTACK_DAMAGE, 5.5D);   // 低攻击力
    }
}
