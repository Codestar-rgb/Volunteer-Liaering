...
/**
 * EntityInfectedCow - Infected cow entity.
 * <p>
 * Former cattle transformed by the parasite. Retains quadrupedal movement
 * but exhibits aggressive behavior and infection spreading capabilities.
 * <p>
 * Stats:
 * - Health: 30.0 (higher than normal cow due to parasite reinforcement)
 * - Attack Damage: 4.0
 * - Speed: 0.25
 * - Armor: 3.0
 */
public class EntityInfectedCow extends EntityInfectedBase {

    private static final EntityDataAccessor<Integer> INFECTION_PROGRESS =
            SynchedEntityData.defineId(EntityInfectedCow.class, EntityDataSerializers.INT);
    ...
    public EntityInfectedCow(EntityType<? extends Monster> type, Level world) {
        super(type, world);
    ...
    protected void registerGoals() {
        ...
        this.targetSelector.addGoal(2, new NearestAttackableTargetGoal<>(this, AbstractHorse.class, true));
    }
    ...
    public boolean doHurtTarget(net.minecraft.world.entity.Entity target) {
        ...
        if (result && target instanceof net.minecraft.world.entity.LivingEntity living) {
            ...
            this.playSound(ModSounds.INFECTED_ATTACK.get(), 1.0f, 0.9f);
        }
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.INFECTED_COW_IDLE.get();
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource source) {
        return ModSounds.INFECTED_COW_HURT.get();
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.INFECTED_COW_DEATH.get();
    }

    @Override
    protected void playStepSound(BlockPos pos, net.minecraft.world.level.block.state.BlockState block) {
        this.playSound(ModSounds.PARASITE_STEP.get(), 0.2f, 0.8f);
    }

    @Override
    public ParasiteType getParasiteType() {
        return ParasiteType.SIM_COW;
    }

    // checkSpawnRules is handled by the static EntityParasiteBase.checkSpawnRules registered via SpawnPlacements
    // checkSpawnObstruction uses the default from Mob
