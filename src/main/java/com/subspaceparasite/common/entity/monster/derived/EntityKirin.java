 ...
 */
public class EntityKirin extends EntityParasiteBase implements IHitboxedEntity {
    ...
    public EntityKirin(EntityType<? extends EntityKirin> type, Level level) {
        super(type, level);
        this.canModRender = 0;
    ...
    private boolean performBlink(BlockPos targetPos) {
        ...
        // Teleport
        boolean success = teleportTo(targetPos.getX() + 0.5, targetPos.getY(), targetPos.getZ() + 0.5);
    ...
    private void completeSummoning() {
        ...
        for (int i = 0; i < VOID_ORB_COUNT; i++) {
            ...
            EntityOrbVoid orb = ModEntities.VOID_ORB.get().create(level());
            if (orb != null) {
    ...
    private class KirinBlinkGoal extends net.minecraft.world.entity.ai.goal.Goal {
        ...
        public void tick() {
            ...
            // Face blink target
            kirin.lookAt(blinkTarget.getX() + 0.5, blinkTarget.getY(), blinkTarget.getZ() + 0.5);
