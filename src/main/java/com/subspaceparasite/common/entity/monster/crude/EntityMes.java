...
/**
 * 融合体 - Crude Stage Parasite
 * 特性：吸收同伴成长进化
 * 
 * 属性设计说明：
 * - MAX_HEALTH: 60.0 - 高血量，成长型单位
 * - MOVEMENT_SPEED: 0.21 - 较低移动速度
 * - ATTACK_DAMAGE: 9.0 - 高攻击力
 */
public class EntityMes extends EntityParasiteBase {

    public EntityMes(EntityType<? extends EntityParasiteBase> entityType, Level worldIn) {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.SUBSRP_ENTITY_MES_AMBIENT.get();
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return ModSounds.SUBSRP_ENTITY_MES_HURT.get();
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.SUBSRP_ENTITY_MES_DEATH.get();
    }
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return EntityParasiteBase.createMonsterAttributes()
            ...
            .add(Attributes.MOVEMENT_SPEED, 0.21D)  // 较低移动速度
            .add(Attributes.ATTACK_DAMAGE, 9.0D);   // 高攻击力
    }
}
