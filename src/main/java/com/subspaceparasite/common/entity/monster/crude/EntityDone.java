...
/**
 * 多足行者 - Crude Stage Parasite
 * 特性：多段移动攀爬能力
 * 
 * 属性设计说明：
 * - MAX_HEALTH: 40.0 - 中等血量，机动型单位
 * - MOVEMENT_SPEED: 0.26 - 中等偏上移动速度
 * - ATTACK_DAMAGE: 6.5 - 中等偏低攻击力
 */
public class EntityDone extends EntityParasiteBase {

    public EntityDone(EntityType<? extends EntityParasiteBase> entityType, Level worldIn) {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.SUBSRP_ENTITY_DONE_AMBIENT.get();
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return ModSounds.SUBSRP_ENTITY_DONE_HURT.get();
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.SUBSRP_ENTITY_DONE_DEATH.get();
    }
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return EntityParasiteBase.createMonsterAttributes()
            .add(Attributes.MAX_HEALTH, 40.0D)      // 中等血量
            .add(Attributes.MOVEMENT_SPEED, 0.26D)  // 中等偏上速度
