package com.subspaceparasite.common.entity.monster.connective;
...
/**
 * EntityVenkrolSIV - Connective Stage SIV (Special Infectious Variant) Parasite
 * <p>
 * Venkrol SIV is a specialized connective parasite with the following characteristics:
 ...
 */
public class EntityVenkrolSIV extends EntityParasiteBase {
    ...
    public EntityVenkrolSIV(EntityType<? extends EntityVenkrolSIV> type, Level world) {
        super(type, world);
        this.xpReward = 25;
        this.setCanPickUpLoot(false);
        this.phaseCreated = EvoPhase.TWO; // Connective phase
    ...
    private void applyInfection(LivingEntity target) {
        if (this.infectionComponent != null) {
            this.infectionComponent.infectEntity(target, INFECTION_LEVEL_ON_HIT);
        }
    }
        ...
        if (this.level() instanceof Level level) {
            ...
            if (!nearbyParasites.isEmpty() && this.getTarget() != null) {
                for (EntityParasiteBase parasite : nearbyParasites) {
                    if (parasite.getTarget() == null && parasite.canSee(this.getTarget())) {
