 ...
 */
public class EntityAssimilatedHuman extends EntityFeralBase {
    ...
    public EntityAssimilatedHuman(EntityType<? extends Monster> type, Level world) {
        super(type, world);
        this.setCanPickUpLoot(false);
        this.xpReward = 10;
        this.setHostEntityType("minecraft:player");
    }
    ...
    private boolean checkEnrageCondition() {
        int nearbyEnemies = (int) this.level().getNearbyEntities(LivingEntity.class, 
            net.minecraft.world.entity.ai.targeting.TargetingConditions.DEFAULT,
            this,
            this.getBoundingBox().inflate(8.0)
        ).stream().filter(e -> !(e instanceof EntityParasiteBase) && e.isAlive()).count();
