...
/**
 * 十字型 A - Crude Stage Parasite
 * 特性：重型近战破甲攻击
 * 
 * 属性设计说明：
 * - MAX_HEALTH: 55.0 - 高血量坦克型单位
 * - MOVEMENT_SPEED: 0.2 - 较慢移动速度
 * - ATTACK_DAMAGE: 10.0 - 高攻击力
 * - 击退抗性：默认值，作为重型单位的特性
 */
public class EntityCruxA extends EntityParasiteBase {

    public EntityCruxA(EntityType<? extends EntityParasiteBase> entityType, Level worldIn) {
    ...
    protected SoundEvent getAmbientSound() {
        return ModSounds.SUBSRP_ENTITY_CRUXA_AMBIENT.get();
    }

    @Override
    protected SoundEvent getHurtSound(DamageSource damageSourceIn) {
        return ModSounds.SUBSRP_ENTITY_CRUXA_HURT.get();
    }

    @Override
    protected SoundEvent getDeathSound() {
        return ModSounds.SUBSRP_ENTITY_CRUXA_DEATH.get();
    }
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return EntityParasiteBase.createMonsterAttributes()
            .add(Attributes.MAX_HEALTH, 55.0D)      // 高血量坦克型
            .add(Attributes.MOVEMENT_SPEED, 0.2D)   // 较慢移动速度
            .add(Attributes.ATTACK_DAMAGE, 10.0D);  // 高攻击力
    }
}
