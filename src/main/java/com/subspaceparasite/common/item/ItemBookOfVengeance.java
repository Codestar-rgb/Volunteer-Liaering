...
public class ItemBookOfVengeance extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level world, Player player, InteractionHand hand) {
        ...
        if (player.isShiftKeyDown()) {
            ...
            // Normal mode: grapple hook — raytrace and pull player toward target
            if (!world.isClientSide()) {
                // Perform raytrace to find target block
                ...
                net.minecraft.world.phys.Vec3 targetPos = net.minecraft.world.phys.Vec3.atCenterOf(blockHit.getBlockPos());
                net.minecraft.world.phys.Vec3 playerPos = player.position();
                net.minecraft.world.phys.Vec3 direction = targetPos.subtract(playerPos).normalize();
                ...
                // Pull player toward target with velocity proportional to distance
                double pullStrength = Math.min(1.5, distance * 0.15);
                ...
                world.playSound(null, player.getBlockX(), player.getBlockY(), player.getBlockZ(),
                        net.minecraft.sounds.SoundEvents.FISHING_BOBBER_THROW, SoundSource.PLAYERS, 1.0f, 0.6f);
            }
            player.swing(hand);
            return InteractionResultHolder.success(stack);
        }
    ...
    public static void doKnockbackBurst(ServerLevel world, LivingEntity source) {
        ...
        // 播放音效
        world.playSound(null, source.getBlockX(), source.getBlockY(), source.getBlockZ(), 
            net.minecraft.sounds.SoundEvents.GENERIC_EXPLODE, SoundSource.PLAYERS, 1.0f, 1.0f);
        ...
        // 对范围内所有实体施加击退
        // Apply knockback to all nearby entities except source
        ...
        for (Entity entity : entities) {
            ...
            // Apply outward + upward knockback to all other entities
            entity.setDeltaMovement(direction.x * KB_STRENGTH, KB_UP, direction.z * KB_STRENGTH);
            entity.hurtMarked = true;
        }
        
        // Give source entity a small upward jump
        source.setDeltaMovement(source.getDeltaMovement().add(0, KB_SELF_JUMP_Y, 0));
