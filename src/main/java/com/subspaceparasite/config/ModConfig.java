...
public class ModConfig {
    ...
    // ========== Merge Settings ==========
    public static final Merge MERGE;

    // ========== Meteor Settings ==========
    public static final Meteor METEOR;
    ...
    public static class Evolution {
        // Phase progression — original SRP has 4 phases (1-4) matching stages
    ...
    public static class Merge {
        public final ForgeConfigSpec.BooleanValue mergeEnabled;
        public final ForgeConfigSpec.IntValue mergeCooldown;
        public final ForgeConfigSpec.DoubleValue mergeChanceBase;
        public final ForgeConfigSpec.DoubleValue mergeChancePerPhase;
        public final ForgeConfigSpec.DoubleValue mergeProximityRange;
        public final ForgeConfigSpec.DoubleValue mergeSearchRange;
        public final ForgeConfigSpec.IntValue mergeCheckInterval;
        public final ForgeConfigSpec.BooleanValue colonyTriggerMerge;
        public final ForgeConfigSpec.BooleanValue transferGenesOnMerge;
        public final ForgeConfigSpec.BooleanValue transferEvolutionOnMerge;

        Merge(ForgeConfigSpec.Builder builder) {
            builder.comment("Entity merge system settings").push("merge");

            mergeEnabled = builder
                    .comment("Whether the entity merge system is enabled")
                    .define("mergeEnabled", true);

            mergeCooldown = builder
                    .comment("Cooldown in ticks after a merge before the result can merge again")
                    .defineInRange("mergeCooldown", 6000, 200, 60000);

            mergeChanceBase = builder
                    .comment("Base chance per check tick for two compatible parasites to merge (0.0-1.0)")
                    .defineInRange("mergeChanceBase", 0.02, 0.0, 1.0);

            mergeChancePerPhase = builder
                    .comment("Additional merge chance per evolution phase level (0.0-1.0)")
                    .defineInRange("mergeChancePerPhase", 0.005, 0.0, 0.5);

            mergeProximityRange = builder
                    .comment("Range in blocks within which two parasites can actually merge")
                    .defineInRange("mergeProximityRange", 2.0, 1.0, 8.0);

            mergeSearchRange = builder
                    .comment("Range in blocks to search for compatible merge partners")
                    .defineInRange("mergeSearchRange", 16.0, 4.0, 64.0);

            mergeCheckInterval = builder
                    .comment("Ticks between merge eligibility checks")
                    .defineInRange("mergeCheckInterval", 100, 20, 600);

            colonyTriggerMerge = builder
                    .comment("Whether colony leaders can trigger merges among colony members")
                    .define("colonyTriggerMerge", true);

            transferGenesOnMerge = builder
                    .comment("Whether the merge result inherits the union of both sources' active genes")
                    .define("transferGenesOnMerge", true);

            transferEvolutionOnMerge = builder
                    .comment("Whether the merge result inherits the sum of both sources' evolution points")
                    .define("transferEvolutionOnMerge", true);
    ...
    public static class Meteor {
        public final ForgeConfigSpec.BooleanValue meteorsEnabled;
        public final ForgeConfigSpec.DoubleValue meteorBaseChance;
        public final ForgeConfigSpec.IntValue meteorCheckInterval;
        public final ForgeConfigSpec.IntValue minMeteorPhase;
        public final ForgeConfigSpec.DoubleValue meteorCraterRadiusMultiplier;
        public final ForgeConfigSpec.IntValue meteorFireCount;
        public final ForgeConfigSpec.IntValue meteorCOTHCount;
        public final ForgeConfigSpec.IntValue meteorParasiteCount;
        public final ForgeConfigSpec.BooleanValue meteorCreateColonies;
        public final ForgeConfigSpec.BooleanValue meteorPermanentInfestation;
        public final ForgeConfigSpec.DoubleValue meteorDamageMultiplier;

        Meteor(ForgeConfigSpec.Builder builder) {
            builder.comment("Meteor falling system settings").push("meteor");

            meteorsEnabled = builder
                    .comment("Whether meteors can fall from the sky")
                    .define("meteorsEnabled", true);

            meteorBaseChance = builder
                    .comment("Base chance per check for a meteor to spawn (0.0-1.0, scaled by phase and difficulty)")
                    .defineInRange("meteorBaseChance", 0.00003, 0.0, 1.0);

            meteorCheckInterval = builder
                    .comment("Ticks between meteor spawn checks")
                    .defineInRange("meteorCheckInterval", 100, 20, 6000);

            minMeteorPhase = builder
                    .comment("Minimum evolution phase (0-4) required for meteors to fall")
                    .defineInRange("minMeteorPhase", 3, 0, 4);

            meteorCraterRadiusMultiplier = builder
                    .comment("Multiplier applied to meteor crater radius")
                    .defineInRange("meteorCraterRadiusMultiplier", 1.0, 0.1, 5.0);

            meteorFireCount = builder
                    .comment("Number of fire blocks to place at impact site per meteor size level")
                    .defineInRange("meteorFireCount", 4, 0, 50);

            meteorCOTHCount = builder
                    .comment("Number of COTH/infested blocks to place at impact site per meteor size level")
                    .defineInRange("meteorCOTHCount", 5, 0, 100);

            meteorParasiteCount = builder
                    .comment("Number of parasite entities to spawn at impact site per meteor size level")
                    .defineInRange("meteorParasiteCount", 1, 0, 20);

            meteorCreateColonies = builder
                    .comment("Whether medium+ meteors create new colonies at impact site")
                    .define("meteorCreateColonies", true);

            meteorPermanentInfestation = builder
                    .comment("Whether large meteors on NIGHTMARE/OBLIVION difficulty create permanent infestation zones")
                    .define("meteorPermanentInfestation", true);

            meteorDamageMultiplier = builder
                    .comment("Global multiplier for meteor impact damage")
                    .defineInRange("meteorDamageMultiplier", 1.0, 0.0, 10.0);
    ...
    static {
        ...
        MERGE = new Merge(BUILDER);
        METEOR = new Meteor(BUILDER);
